// Get the input equation to balance
  ...
  try {
    ...
    if (e instanceof ParseError) {
      ...
      if (end <= formulaStr.length) {
        codeOutElem.append(
          createElem("span", formulaStr.substring(start, end), "error")
        );
  ...
  toHtml(coefs) {
    ...
    function termsToHtml(terms) {
      ...
      for (const term of terms) {
        ...
        if (coef != 0) {
          if (head) head = false;
          else node.append(createSpan("plus", " + "));
          if (coef != 1) {
            let span = createSpan(
              "coefficient",
...
class Term {
  constructor(items, charge) {
    if (items.length == 0 && charge != -1) throw new RangeError("Invalid term"); // Electron case
  ...
  toHtml() {
    ...
    if (this.items.length == 0 && this.charge == -1) {
      ...
      for (const item of this.items) node.append(item.toHtml());
      if (this.charge != 0) {
        let s;
...
class Group {
  ...
  toHtml() {
    ...
    for (const item of this.items) node.append(item.toHtml());
    node.append(")");
    if (this.count != 1) node.append(createElem("sub", this.count.toString()));
    return node;
  }
}
// A chemical element.
...
function createElem(tagName, text, error = false) {
  let result = document.createElement(tagName);
  if (error) result.className = "errorCharacter";
