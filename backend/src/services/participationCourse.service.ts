...
export default class ParticipationCourseService {
  ...
  static addParticipationCourse = async (participationCourseArgs: IParticipationCourse) => {
    const [participationCourse] = await ParticipationCourse.insertMany([participationCourseArgs]);
    return participationCourse;
  };

  static updateParticipationCourse = async (participationCourseArgs: IParticipationCourse) => {
  ...
  static removeParticipationCourse = async (_id: string) => {
    const participationCourse = await ParticipationCourse.findByIdAndDelete(_id);
